...
if __name__ == "__main__":
	...
	if not multiScan:
		if oneUnload: # Just one type	
			...
			#ltVec, runPair,holdPair,ltVec2 = pair_runs(runBreaks,reducedRunS,anlz)
			ltVec, runPair,holdPair,ltVec2 = pair_runs(runBreaks,reducedRunS,anlz)
			ltVec,runPair,redRunS_2 = pair_runs_from_file(reducedRunS,anlz,'/home/frank/Downloads/emf_pairs.csv')
		...
		if oneUnload: # So either singles OR coincidence
			...
			for k,rRun in enumerate(reducedRunS):
				...
				if outS.plotPSE:
					...
					plot_gen += histogram_phasespace(rRun,plot_gen,[20,1550])
					plot_gen += histogram_mean_arr(rRun,plot_gen,[20,1550])
				if outS.plotSig:
